...
class Website(models.Model):
    ...
    def sale_get_order(self, force_create=False, code=None, update_pricelist=False, force_pricelist=False):
        ...
        sonumber = False
        if "/shop/cart" in current_url and len(current_url.split('?')) >= 2:
            sonumber = current_url.split('?')[1].split('=')[1]

        self.ensure_one()
        partner = self.env.user.partner_id
        sale_order_id = request.session.get('sale_order_id')
        if sonumber:
            so_id = request.env['sale.order'].sudo().search([('name', '=', "SO"+sonumber)])
            if so_id and so_id.state == 'draft':
                sale_order_id = so_id.id
                request.session['sale_order_id'] = so_id.id
        check_fpos = False
        ...
        #change website partner to order partner if seller login
        if sale_order and request.env.user.is_seller:
        ...
        if sale_order and sale_order.state == 'draft' and sale_order.seller_partner_id and sale_order.seller_partner_id != sale_order.partner_id:
            pricelist = sale_order.pricelist_id.sudo()
            partner = sale_order.seller_partner_id
            so_data = self._prepare_sale_order_values(partner, pricelist)
            sale_order.with_company(request.website.company_id.id).with_user(SUPERUSER_ID).write(so_data)

        if sale_order:
            return sale_order.with_context(warehouse=sale_order.warehouse_id.id) if sale_order else sale_order
        return sale_order
