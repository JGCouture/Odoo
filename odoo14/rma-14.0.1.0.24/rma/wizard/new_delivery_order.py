...
class NewDeliveryOrderWizard(models.TransientModel):
    ...
    def default_get(self, default_fields):
        ...
        if rma_obj:
            ...
            company_id = rma_obj.order_id.company_id
            picking_type = rma_obj.order_id.warehouse_id.out_type_id
            if not picking_type:
                picking_type = self.env['stock.picking.type'].search([('code', '=', 'outgoing'), ('company_id', '=', company_id.id)], limit=1)
                if not picking_type:
                    raise Warning('Make sure you have at least an outgoing picking type defined')
