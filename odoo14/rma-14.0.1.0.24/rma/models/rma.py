...
class RmaRma(models.Model):
    ...
    def _get_default_stage(self):
        ...
        rma_stages = rma_stage_obj.search([('related_status', '=', 'new'),('company_id','=', self.env.company.id)])
        if rma_stages:
    ...
    def _get_term_condition(self):
        return self.env.company.rma_term_condition
    ...
    def quantity_onchange(self):
        if self.product_id:
            ...
            if sale_order_line_obj:
                line_qty = sum(sale_order_line_obj.mapped('product_uom_qty'))
