...
class SaleOrderline(models.Model):
    ...
    def _check_eligibility_for_rma(self):
        for obj in self:
            create_date = None
            if self.order_id.company_id.days_for_rma:
                if self.order_id.company_id.rma_day_apply_on and self.order_id.company_id.rma_day_apply_on == "so_date":
                    create_date = obj.create_date.date()
                if self.order_id.company_id.rma_day_apply_on and self.order_id.company_id.rma_day_apply_on == "do_date":
                ...
                days = timedelta(self.order_id.company_id.days_for_rma or 0)
    ...
    def get_rma_t_and_c(self):
        x = self.order_id.company_id.rma_term_condition
        return x
