...
class StockPicking(models.Model):
    ...
    #     if res.shipstation_order_id:
    #         pick_ids = self.sudo().search([('id', '!=', res.id), ('shipstation_order_id', '=', res.shipstation_order_id)])
    #         if pick_ids:
    #             res.shipstation_order_id = False
    #     return res

    def _action_done(self):
        for rec in self:
            pick_ids = self.sudo().search([('id', '!=', rec.id), ('shipstation_order_id', '=', rec.shipstation_order_id)])
            if pick_ids:
                rec.shipstation_order_id = False
