...
class DeliveryCarrier(models.Model):
    ...
    def get_total_weight(self, weight=1):
        #this method modified to fix the wieght issue while using multi language.
        ...
        if self.weight_uom == "pounds" and uom_id.with_context(lang='en_US').name in ['lb','lbs']:
            return round(weight, 3)
        elif self.weight_uom == "pounds" and uom_id.with_context(lang='en_US').name == 'kg':
            return round(weight * pound_for_kg, 3)
        elif self.weight_uom == "ounces" and uom_id.with_context(lang='en_US').name in ['lb','lbs']:
            return round(weight * ounce_for_lb, 3)
        elif self.weight_uom == "ounces" and uom_id.with_context(lang='en_US').name == 'kg':
            return round(weight * ounce_for_kg, 3)
        elif self.weight_uom == "grams" and uom_id.with_context(lang='en_US').name == 'kg':
    ...
    def check_order_data(self, order):
        lines_without_weight = order.order_line.filtered(
            lambda line_item: line_item.product_id and not line_item.product_id.type in ['service',
    ...
    def create_or_update_order(self, picking):
        ...
        total_weight = self.get_total_weight(weight)
        _logger.info("Total Weight :{}".format(total_weight))
    ...
    def shipstation_send_shipping(self, pickings):
        _logger.info("sheipping------->>..: %s" % (pickings))
