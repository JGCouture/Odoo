...
class AccountChartOfAccountReport(models.AbstractModel):
    ...
    def _get_lu_xml_2_0_report_values(self, options, avg_nb_employees=1, size='small',
        ...
        # Format everything
        for key in account_fields.keys():
    ...
    def _get_account_fields(self, options, account_dict, year):
        ...
        for account_code in account_dict.keys():
            ...
            if (account_fields.get(account_dict[account_code]["debit"]) or account_fields.get(account_dict[account_code]["credit"])) \
                ...
                if float_compare(net, 0.0, 2) > 0:
                    ...
                    if account_dict[account_code]["credit"] in account_fields.keys():
                        del account_fields[account_dict[account_code]["credit"]]
                else:
                    account_fields[account_dict[account_code]["credit"]] = -net
                    if account_dict[account_code]["debit"] in account_fields.keys():
