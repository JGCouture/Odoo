...
class SwedishBlackBoxDriver(SerialDriver):
        ...
        try:
            ...
            with serial_connection(device["identifier"], protocol) as connection:
                ...
                if len(response) > 1 and response[3] == "SRX":
                    device["UnitId"] = cls._get_unit_id(cls, connection)
                    if response[4] != "0":
                        _logger.warning(
    ...
    def _get_unit_id(cls, connection):
        ...
        """
        try:
            response = cls._request_action(cls, "IQ", connection)
            if response[3] == "IR":
                return response[4]
            else:
                return False
    ...
    def _register_receipt(self, data):
        ...
        response = self._request_action(self, request, self._connection)
