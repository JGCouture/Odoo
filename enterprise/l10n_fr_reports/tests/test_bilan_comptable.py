...
class TestBilanComptable(TestAccountReportsCommon):
    ...
    def test_bilan_comptable_bank_actif_passif(self):
        ...
        line_id = self.env.ref('l10n_fr_reports.account_financial_report_line_03_5_4_fr_bilan_actif').id
        ...
        line_id = self.env.ref('l10n_fr_reports.account_financial_report_line_03_3_3_fr_bilan_passif').id
        options['unfolded_lines'] = [line_id]
        lines = report._get_lines(options, line_id=line_id)
        self.assertLinesValues(
            lines,
